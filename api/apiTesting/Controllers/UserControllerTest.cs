...
// "new UserController(mock)" test could never actually observe a 401.
// IUserService is swapped for a mock in DI, so no real database is touched.
public class UserControllerTest
{
    ...
    [Test]
    public async Task GetAllUsers_WithoutAuthToken_Returns401()
    {
        // Arrange
        ...
        _userServiceMock.Verify(s => s.GetAll(), Times.Never);
    }
    ...
    {
        // Arrange
        _userServiceMock.Setup(s => s.GetAll()).ThrowsAsync(new InvalidOperationException("boom"));
        var client = _factory.CreateClient();
    ...
    [Test]
    public async Task GetAllUsers_WithValidToken_ReturnsOkFromMockedService()
    {
        // Arrange
        _userServiceMock.Setup(s => s.GetAll()).ReturnsAsync([]);
        var client = _factory.CreateClient();
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", BuildAccessToken());
