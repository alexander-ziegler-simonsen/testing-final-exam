...
// "new StorageController(mock)" test could never actually observe a 401.
// IStorageService is swapped for a mock in DI, so no real database is touched.
public class StorageControllerTest
{
        ...
        _factory = new WebApplicationFactory<Program>()
            .WithWebHostBuilder(builder =>
            {
                ...
                builder.ConfigureTestServices(services =>
                {
                    ...
                    services.AddScoped(_ => _storageServiceMock.Object);
                });
            });
    }
    ...
    [Test]
    public async Task GetAllMedicationStorages_WithoutAuthToken_Returns401()
    {
        // Arrange
        ...
        _storageServiceMock.Verify(s => s.GetAll(), Times.Never);
    }

    [Test]
    public async Task GetAllMedicationStorages_WhenServiceThrows_Returns500()
    {
        // Arrange
        _storageServiceMock.Setup(s => s.GetAll()).ThrowsAsync(new InvalidOperationException("boom"));
        var client = _factory.CreateClient();
    ...
    [Test]
    public async Task GetAllMedicationStorages_WithValidToken_ReturnsOkFromMockedService()
    {
        // Arrange
        _storageServiceMock.Setup(s => s.GetAll()).ReturnsAsync([]);
        var client = _factory.CreateClient();
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", BuildAccessToken());
        ...
        _storageServiceMock.Verify(s => s.GetAll(), Times.Once);
    }
