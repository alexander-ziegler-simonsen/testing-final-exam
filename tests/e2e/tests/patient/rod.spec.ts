...
test('test', async ({ page }) => {
  ...
  await page.getByTestId('login-username-input').fill('bill');
  await page.getByTestId('login-username-input').press('Tab');
  await page.getByTestId('login-password-input').fill('passwordHere');
  await page.getByTestId('login-submit-button').click();

  // check we got to the right page
  await expect(page).toHaveURL(/\/app\/overview/);

  // check visablity of dashboard elements
  // TODO - fix this to use getByTestId instead of getByText
  await expect(page.getByText('treatmentopen')).toBeVisible();
  
  // checks that are not core part of the test, but should not be visible to a patient
  await expect.soft(page.getByText('departmentsopen')).not.toBeVisible();
  await expect.soft(page.getByText('department staffopen')).not.toBeVisible();
  await expect.soft(page.getByText('facilitiesopen')).not.toBeVisible();
  await expect.soft(page.getByText('missing medsopen')).not.toBeVisible();
  await expect.soft(page.getByText('medicin storageopen')).not.toBeVisible();
  await expect.soft(page.getByText('find medicin priceopen')).not.toBeVisible();
  await expect.soft(page.getByText('patientsopen')).not.toBeVisible();
  await expect.soft(page.getByText('staffopen')).not.toBeVisible();
  await expect.soft(page.getByText('shiftsopen')).not.toBeVisible();
  await expect.soft(page.getByText('room bookingopen')).not.toBeVisible();
